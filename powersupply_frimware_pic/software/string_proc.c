...
T_UBYTE Read_data_from_address8(T_ULONG lul_address_request)
{
    ...
    /*T_UBYTE *pub_content = 0;
    direct access to pic memory is not posible*/
    /*pub_content = (T_UBYTE *)ul_address_request;
    lub_result = *pub_content;*/
    FSR = (T_UBYTE)lul_address_request;
    if ((lul_address_request & 0x100) == 0x100)
    {
        IRP = 1;
    }
    else
